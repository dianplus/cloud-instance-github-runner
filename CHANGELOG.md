...
- Renamed the env var carrying the new instance's runner name to SPOT_RUNNER_NAME.
- Hardened the SPOT_RUNNER_NAME contract: both consumer steps forward-pinned and RUNNER_NAME re-declaration reverse-pinned by structural tests, a wait-side precedence mirror added (an online host-agent-named runner must never satisfy the wait step), error messages now name SPOT_RUNNER_NAME, and the RUNNER_TEMP comment matches the documented conditional cleaning (files the runner user cannot delete survive)
  RUNNER_NAME is injected into every step by the Actions runner with the host agent's own name and cannot be overridden at step level, so on a self-hosted runner the instance registered under the host agent name and config.sh --replace took over its registration
