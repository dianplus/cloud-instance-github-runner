...
| `runner_version`                     | GitHub Actions Runner 版本。留空（推荐）时在 setup 阶段自动解析最新非预发布版本（先经所配代理探测，GitHub-hosted 场景自动改试直连）；仅需要可复现舰队时显式钉扎版本。                                                                                                                                                                                                              | 自动（latest）                           |
| `runner_wait_timeout`                | 可选——无需显式设置。等待 runner 上线的总秒数（轮询间隔 10s；该值为总窗口而非单次超时；实例创建的 candidates/磁盘类别重试不受此参数影响）。默认 420 与经 VPC 代理冷启动的 smoke 实测时长一致；代理更慢时按需调大。                                                                                                                                                                  | `420`                                    |
| `watchdog_stop_window_seconds`       | runner 持续不活跃的最长秒数，达到后实例自毁（watchdog 停止判定窗；任何 active 探测都会重置计数，故此为最长等待而非强制守望）。未设置（推荐）保持 watchdog 默认（24 次探测 × 5s = 2 分钟），并保留自定义镜像预置的 `STOP_CONFIRMATIONS_REQUIRED`；设置后两者均被覆盖。代理下 runner 自更新或服务重启较慢时调大；窗口加宽的代价是 job 后的空转时间，由 `instance_ttl_minutes` 兜底。 | 未设置（2 分钟）                         |
...
| `runner_version`        | 实际使用的 runner 版本（显式钉扎 / 解析的 latest / fallback） | 无  |
| `runner_version_source` | `runner_version` 的确定方式：`pin` / `latest` / `fallback`    | 无  |
...
- 检查 runner-watchdog 日志是否显示停止确认窗口（24 次连续 inactive 探测，即 2 分钟窗口——仍在倒数时会打印 `confirmed-inactive n/24`）
