  ...
  setup:
    ...
    steps:
      - name: Setup Aliyun ECS Spot Runner
        id: setup-runner
        uses: dianplus/cloud-instance-github-runner@v1.6.0
        with:
...
| `runner_version`                     | GitHub Actions Runner version. Empty (recommended) auto-resolves the latest non-prerelease release at setup time (probed inside the configured proxy scope, retried direct for GitHub-hosted setups); pin an explicit version only for reproducible fleets.                                                                                                                                                                                                                                                                              | auto (latest)                            |
| `runner_wait_timeout`                | Optional — no need to set explicitly. Total seconds the action waits for the runner to come online (poll interval 10s; this is the TOTAL window, not a per-attempt timeout; instance-creation retries via candidates/disk-category are governed separately and unaffected). Default 420 matches the smoke-tested cold-bootstrap time through the VPC proxy; raise it for slower proxies.                                                                                                                                                 | `420`                                    |
| `watchdog_stop_window_seconds`       | Max seconds of continuous runner inactivity before the instance self-destructs (watchdog stop-verdict window; any active probe resets it, so this is a maximum, not a forced wait). Unset (recommended) keeps the watchdog default (24 probes x 5s = 2min) and preserves any `STOP_CONFIRMATIONS_REQUIRED` pre-baked into a custom image; setting it overrides both. Raise for proxies where runner self-updates or service restarts run long; the cost of a wider window is idle time after the job, bounded by `instance_ttl_minutes`. | unset (2min)                             |
...
| `spot_duration`                      | Spot protection period in hours: 1 = no system reclamation in the first hour; 0 = no protection, reclaimable anytime. Only 0 or 1. Billed by second regardless. Passed explicitly — the implicit default did not reliably protect in production.                                                                                                                                                                                                                                                                                         | `1`                                      |
| `spot_strategy`                      | Bidding strategy: SpotWithPriceLimit (default) bids market price × spot_price_multiplier as SpotPriceLimit; SpotAsPriceGo follows the market automatically — the multiplier/limit do not affect bidding (no --SpotPriceLimit sent) but inputs are still fully validated. Explicit SpotAsPriceGo takes priority over the multiplier.                                                                                                                                                                                                      | `SpotWithPriceLimit`                     |
...
| `runner_version`        | The runner version actually used (explicit pin, resolved latest, or fallback) | n/a |
| `runner_version_source` | How `runner_version` was determined: `pin` / `latest` / `fallback`            | n/a |
...
Spot bidding is tunable via `spot_price_multiplier` (bid = market price × multiplier), the system-reclamation protection window via `spot_duration` (spot instances are billed by second regardless of the protection period), and the bidding strategy itself is selectable via `spot_strategy`.
...
- Check the runner-watchdog log shows the stop confirmation window (24 consecutive inactive probes, i.e. a 2-minute window — still counting down prints `confirmed-inactive n/24`)
